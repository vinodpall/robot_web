            ...
            <div class="nav-content-wrapper">
              <div class="track-record-toolbar">
                ...
                <button 
                  ...
                  :class="[isTrackRecording ? 'map-btn-danger' : (!hasNavEnabled || isTrackRunning ? 'map-btn-disabled-visual' : 'map-btn-secondary')]"
                  :disabled="isTrackRunning || !hasNavEnabled"
...
import { clearInsOriginCache, resolveEffectiveGnssOrigin } from '@/utils/insOrigin'
...
const handleTrackRecord = () => {
  if (isTrackRunning.value || !hasNavEnabled.value) {
    return
  }
...
const hasNavEnabled = computed(() => navigationEnabled.value || insEnabled.value || msfEnabled.value)
  ...
  showConfirmDialog({
    ...
    onConfirm: async () => {
      try {
        ...
        const res: any = await navigationApi.setInsOrigin(robotId, {
          action: 1
        })
        
        // 解析返回数据（兼容直接返回或嵌套在 response 中的结构）
        const payload = res?.response || res
        const msgObj = payload?.msg || res?.msg

        // 检查返回结构是否有报错
        if (msgObj) {
          if (typeof msgObj === 'object') {
            const { result, error_msg, error_code } = msgObj
            if (result === 0 || (error_code !== undefined && error_code !== 0)) {
              const errMsg = error_msg || res?.message || 'INS初始化失败'
              console.error('INS初始化失败:', errMsg, res)
              showErrorMessage(errMsg)
              return
            }
          } else if (typeof msgObj === 'string' && msgObj !== 'success') {
            showErrorMessage(msgObj)
            return
          }
        }

        // 成功提示：优先展示后端返回的 message 或 error_msg 文案
        const successMsg =
          (typeof msgObj === 'object' && msgObj?.result === 1 && msgObj?.error_msg) ||
          res?.message ||
          'INS初始化成功'

        showSuccessMessage(successMsg)
        clearInsOriginCache(robotId)
        void robotStore.syncInsOrigin(robotId)
      } catch (err: any) {
        console.error('INS初始化失败:', err)
        const errMsg =
          err?.msg?.error_msg ||
          err?.response?.msg?.error_msg ||
          (typeof err?.msg === 'string' ? err.msg : null) ||
          err?.response?.data?.msg?.error_msg ||
          err?.response?.data?.response?.msg?.error_msg ||
          (typeof err?.response?.data?.msg === 'string' ? err.response.data.msg : null) ||
...
const overlayNavTrackTrajectory = async (trackName: string) => {
  ...
  try {
    ...
    if (navAmapInstance && navAmapApiRef) {
      ...
      const gnssOrigin = await resolveEffectiveGnssOrigin(selectedNavMap.value, currentRobotId, {
    ...
    navRobotMarker = new AMap.Marker({
      ...
      content: `
        <div class="robot-location-indicator">
          ...
          <defs>
            <linearGradient id="navManageArrowLeft" x1="6" y1="20" x2="20" y2="20" gradientUnits="userSpaceOnUse">
                <stop offset="0%" stop-color="#ff77ed"/>
                <stop offset="100%" stop-color="#e024c3"/>
              </linearGradient>
              <linearGradient id="navManageArrowRight" x1="20" y1="20" x2="34" y2="20" gradientUnits="userSpaceOnUse">
                <stop offset="0%" stop-color="#c026d3"/>
                <stop offset="100%" stop-color="#86198f"/>
              </linearGradient>
            <filter id="navManageShadow" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="2" stdDeviation="2.5" flood-color="#000000" flood-opacity="0.5"/>
            </filter>
          </defs>
          <path d="M20 4L7 35L20 27L33 35L20 4Z" fill="#c026d3" stroke="#ffffff" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round" filter="url(#navManageShadow)"/>
          <path d="M20 4L7 35L20 27Z" fill="url(#navManageArrowLeft)"/>
          <path d="M20 4L20 27L33 35Z" fill="url(#navManageArrowRight)"/>
